        ...
        if( $('#myDoughnutChart').length > 0 )
        {
            ...
            const centerTextPlugin = {
                ...
                afterDatasetsDraw(chart) {
                    ...
                    const textY = height / 2 + (fontSize * 15); // Adjust vertical positioning to move it slightly down

                    ctx.fillText(`${Math.round(aiPercentage)}%`, textX, textY); // Display percentage in center
                    ctx.restore();
                }
            };
